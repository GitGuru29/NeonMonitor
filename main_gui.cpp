...
void DrawRadialProgress(const char* label, float value, float max, ImVec2 center, float radius, ImVec4 color, const char* format, const char* subtext1 = "", const char* subtext2 = "") {
    ...
    if (subtext1[0] != '\0') {
        ...
        draw_list->AddText(NULL, 13.0f, s1_pos, ImColor(255, 255, 255), subtext1);
    }

    if (subtext2[0] != '\0') {
        ImVec2 s2_size = ImGui::CalcTextSize(subtext2);
        ImVec2 s2_pos = ImVec2(center.x - s2_size.x * 0.5f, center.y + 15); 
        draw_list->AddText(NULL, 13.0f, s2_pos, ImColor(255, 255, 255), subtext2);
    }
...
int main(int, char**) {
    ...
    SDL_Window* window = SDL_CreateWindow("System Monitor V6.1 (Connectivity Check)", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1200, 800, window_flags);
    ...
    while (!done) {
        ...
        if (SDL_GetTicks() - last_tick > 1000) {
            ...
            c_online = system.IsConnected(); // <--- UPDATE CONNECTIVITY
        ...
        ImGui::Text("SYSTEM_MONITOR // V6.1 // SMART_NET");
        ImGui::Separator();
        ImGui::Spacing();
        ...
        if (ImGui::BeginTable("GaugesTable", 4)) {
            ...
            if (c_bat >= 0) {
                ...
                DrawRadialProgress("BATTERY", (float)c_bat, 100.0f, center, radius, bat_col, "%.0f%%");
                ImGui::Dummy(ImVec2(0, cell_height));
            }
